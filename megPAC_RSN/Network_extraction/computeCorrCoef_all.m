function [avgCorrCoefFile,nTime] = computeCorrCoef_all_MS_all_subject(dataPath, sourceFiles, FWHM,Med_state,Study_state)
...
% And save to file
if 1
    sourceResults.Comment = sprintf(['Corr_map_all_Subjects_' Med_state '_' Study_state]);
    ...
    avgCorrCoefFile = ['results_AverageCorrmap_all_Subjects_' Med_state '_' Study_state '.mat'];
    avgCorrCoefFile = fullfile(dataPath,avgCorrCoefFile);
    save(avgCorrCoefFile, '-struct', 'sourceResults','-v7.3')
else
    avgCorrCoefFile = '';
end
...
avgCorrCoef.Comment = sprintf(['Corr_maps_row_Z_scores_all_Subjects_' Med_state  '_' Study_state]);
%ZavgCorrCoefFile = strrep(sourceFiles{ifile},'results_', 'results_000_zscoreAverageCorrmap');
ZavgCorrCoefFile = ['results_zscoreAverageCorrmap_all_Subjects_' Med_state '_' Study_state '.mat'];

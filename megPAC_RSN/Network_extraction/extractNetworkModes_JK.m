function [networkModesFile] = extractNetworkModes_MS(avgCorrCoefFile, iRun ,subjects, study_state, varargin)
...
%     noise_ttmp = normr(noise_ttmp); 
    ...
    if iSave
        ...
        clusterResults.Comment = ...
            sprintf(['JK | NOISE MODES | %d noise modes removed | %d modes  | without ' subjects{iRun} ' | ' study_state], nprojNoise, nDims);
...
%         clusterFile = strrep(avgCorrCoefFile,'results_', 'results_AllSubjects_0000_noiseModes.mat');
        clusterFile = ['results_AllSubjects_0000_noiseModes_wihtout' subjects{iRun} '_' study_state '.mat'];
    ...
    if iSave
        ...
        % And save
        clusterResults.Comment = ...
            sprintf(['JK | NOISE RESIDUALS | %d noise modes removed | without ' subjects{iRun} ' | ' study_state], nprojNoise);
        clusterResults.Time =  1:size(clusterResults.ImageGridAmp, 2);
        clusterResults.ImageGridTime = 1:size(clusterResults.ImageGridAmp, 2);
%         clusterFile = strrep(avgCorrCoefFile,'results_', 'results_AllSubjects_0100_projectedNoiseCorrCoef.mat');
        clusterFile = ['results_AllSubjects_0100_projectedNoiseCorrCoef_without_ ' subjects{iRun} '_' study_state '.mat'];
        save(clusterFile, '-struct', 'clusterResults')
        clusterResults.ImageGridAmp = [];
    end
...
clusterResults.Comment = sprintf(['JK | Mean Conn. pattern, Residuals, SIGNAL MODES | %d noise modes, %d signal modes | without ' subjects{iRun}], nprojNoise, nproj);
clusterResults.Time =  1:size(clusterResults.ImageGridAmp, 2);
clusterResults.ImageGridTime = 1:size(clusterResults.ImageGridAmp, 2);
% networkModesFile = strrep(avgCorrCoefFile,'results_', 'results_AllSubjects_100_signalModes.mat');
networkModesFile = ['results_AllSubjects_100_signalModes_without_ ' subjects{iRun} '_' study_state '.mat'];
...
clusterResults.Comment = sprintf(['JK | positive & negative, SIGNAL MODES | %d signal modes | without ' subjects{iRun}], nproj);
clusterResults.Time =  1:size(clusterResults.ImageGridAmp, 2);
clusterResults.ImageGridTime = 1:size(clusterResults.ImageGridAmp, 2);
% networkModesFile = strrep(avgCorrCoefFile,'results_', 'results_AllSubjects_000_posandnegSignalModes.mat');
networkModesFile = ['results_AllSubjects_000_posandnegSignalModes_without_ ' subjects{iRun} '_' study_state '.mat'];
save(networkModesFile, '-struct', 'clusterResults')
...
clusterResults.Comment = sprintf(['JK | SIGNAL MODES LABELS | %d modes  | without ' subjects{iRun}], nproj);
clusterResults.Time =  1:size(clusterResults.ImageGridAmp, 2);
clusterResults.ImageGridTime = 1:size(clusterResults.ImageGridAmp, 2);
% networkModesFile = strrep(avgCorrCoefFile,'results_', 'results_AllSubjects_001_labelsSignalModes.mat');
networkModesFile = ['results_AllSubjects_001_labelsSignalModes_without_ ' subjects{iRun} '_' study_state '.mat'];
save(networkModesFile, '-struct', 'clusterResults')

function megPAC_bst_parallel( F, sRate, run_subject, lowfreqency, GoodChannel,OutputPath )
...
tmp=(resample(F(1,:),1,round(sRate/10)));
sRate2 = sRate/round(sRate/10); %new sampling rate

HRF_conv_all=zeros(size(F,1),size(tmp,2));
clear temp
matlabpool('open',6)
for k=1:10
    Data=F((k-1)*100+1:k*100,:);
    HRF_conv=zeros(100,size(tmp,2));
parfor source=1:size(Data,1) %Loop over all sources in F
     lf_g=[];
...
HRF_conv_all((k-1)*100+1:k*100,:)=HRF_conv;

end
SaveResults(HRF_conv_all, [run_subject 'megPAC_lf_2-48_trough_gammaamp_resamples_bst'], 'lf 2-48 trough gamma amp bst' ,sRate2,GoodChannel,OutputPath)
